...
class package extends control
{
        ...
        if($packages and $status == 'installed')
        {
            ...
            if(isset($results->extensions))
            {
                ...
                foreach($remoteReleases as $release)
                {
                    ...
                    if(isset($release->latestRelease) and $package->version != $release->latestRelease->releaseVersion and $this->package->checkVersion($release->latestRelease->ranzhiCompatible))
                    {
    ...
    public function install($package, $downLink = '', $md5 = '', $type = '', $overridePackage = 'no', $ignoreCompatible = 'no', $overrideFile = 'no', $agreeLicense = 'no', $upgrade = 'no')
    {
        ...
        /* Get condition. e.g. ranzhi|depends|conflicts. */
        ...
        $incompatible = $condition->ranzhi['incompatible'];
        ...
        $ranzhiCompatible = $condition->ranzhi['compatible'];
        if(!$this->package->checkVersion($ranzhiCompatible) and $ignoreCompatible == 'no')
        {
